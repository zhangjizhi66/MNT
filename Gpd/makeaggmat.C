...
void makeaggmat()
{
    ...
    tree->Draw("epT:ed>>ggT(2000,0,2000,2000,0,2000)","","colz");
    tree->Draw("epP:ed>>ggP(2000,0,2000,2000,0,2000)","","colz");
    auto hgg=(TH2D*)gROOT->FindObject("ggT");
    auto hggP=(TH2D*)gROOT->FindObject("ggP");
    auto hx=(TH1D*)hgg->ProjectionX("xTpj");

    TSpectrum *sx= new TSpectrum(500);
    Int_t nfoundx=sx->Search(hx,2,"",0.1);
    auto hbx=sx->Background(hx,8,"nosmoothingsame");
    hbx->SetName("xTpjBg");
    hbx->SetTitle("xTpjBg");
    ...
    auto hby=sy->Background(hy,8,"nosmoothingsame");
    hby->SetName("yTpjBg");
    hby->SetTitle("yTpjBg");
