...
Short_t cosPx,cosTx,cosPy,cosTy; // cos value times 10000
Short_t pP_new,pT_new;

int main(int argc, char** argv)
{
    if (argc != 3){
        printf("USAGE: ./makeGpproot2 runid_1 runid_2");
        exit(EXIT_FAILURE);
    }
    ...
    const char *filename = Form("ROOT/Gpp2_%d_%d.root",atoi(argv[1]),atoi(argv[2]));
