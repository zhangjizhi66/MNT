...
#define ENERGYMAX 2000  //keV
#define FILENAME_M4B  "co59.m4b"
#elif DATAFORMAT == DATAFORMAT_ROOT//如果采用 root 格式数据，则修改以下定义
#define FILENAME_ROOT "ROOT/Gpppmats2_192.root"
#define ROOTTH2TYPE  TH2I  //可选择 TH2I TH2F TH2D
#define ROOTTH2NAME  "gg"
#endif
...
void gggm()
{
    cout<<"Opening TH2 "<<ROOTTH2NAME<<" from root file "<<FILENAME_ROOT<<" ..."<<endl;
  ...
  fin=new TFile(FILENAME_ROOT);
  // h2=(ROOTTH2TYPE*)fin->Get(ROOTTH2NAME);
  // h2->ClearUnderflowAndOverflow();
#endif

  h1 = (TH1I*)fin->Get("g");
  hxmax = h1->GetXaxis()->GetXmax();
  hxmin = h1->GetXaxis()->GetXmin();
  tpj();
}
...
void gate(int gpeak1,double wgpeak=140)
{
  gpeak=gpeak1;
  peakbin.clear();
  std::cout<<"Gate: left-click on the left/right bins"<<std::endl;
...
void gshow()
{
  ...
  newcanvas();//create ca[ic];
    if(!htmp) htmp=new TH2I("hgtmp","",h1->GetXaxis()->GetNbins(),0,h1->GetXaxis()->GetXmax(),h1->GetXaxis()->GetNbins(),0,h1->GetXaxis()->GetXmax());
  TH2I *hgg = new TH2I("hgg","",h1->GetXaxis()->GetNbins(),0,h1->GetXaxis()->GetXmax(),h1->GetXaxis()->GetNbins(),0,h1->GetXaxis()->GetXmax());
  ...
  for(auto is=peakbin.begin();is!=peakbin.end();is++)
    {
      //htmp->Reset();
      htmp = (TH2I*)fin->Get(Form("gg_%d",2*int(*is)-1));
      hgg->Add(hgg,htmp,1,1);
    }
  for(auto is=bgbin.begin();is!=bgbin.end();is++)
    {
      //htmp->Reset();
      htmp = (TH2I*)fin->Get(Form("gg_%d",2*int(*is)-1));
      hgg->Add(hgg,htmp,1,-weight);    
    }
  ...
  TFile *opf = new TFile("ROOT/Gpppmat2.root","RECREATE");
  opf->cd();
  hgg->Write();
  opf->Close();
}
