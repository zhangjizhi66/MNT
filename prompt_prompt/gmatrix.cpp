void gmatrix()
{
    TFile *fin=new TFile("ROOT/Gpp.root");
    ...
    TCut cut = "np==2 && (eTx<401 || (eTx>411 && eTx<572) || eTx>582) && (eTy<401 || (eTy>411 && eTy<572) || eTy>582)";
    ...
    tree->Draw("ePx>>hg2x(2000,0,2000)",cut);
    hg2x = (TH1S*)gROOT->FindObject("hg2x");
    
    tree->Draw("ePy:ePx>>hg2xy(2000,0,2000,2000,0,2000)",cut);
    hg2xy = (TH2S*)gROOT->FindObject("hg2xy");

    //"nosmoothing" option to avoid overestimation of the background 
    hg2xb=(TH1S*)sa->Background(hg2x,12,"nosmoothing");
    hg2xb->SetName("hg2xb");
